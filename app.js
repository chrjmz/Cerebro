...
// --- Tongue extra clamps (lateral y subida) ---
const TONGUE_MAX_SIDE_M = 0.0020; // |ΔX| máx desde reposo (lateral)
const TONGUE_MAX_UP_M   = 0.0040; // subida Y máx (opcional)
...
const LIP_OUTWARD_CLAMP = {
  enabled: true,
  up: { maxDX: 0.006, maxDZ: 0.004 },
  down: { maxDX: 0.005, maxDZ: 0.004 }
};
...
async function calibrateMicGain(ms = 700) { /* keep placeholder */ }
...
// --- LENGUA (segura + límite de bajada/rotación + clamps X/Y) ---
...
function clampTongueLocalToRest(node) {
  if (!node) return;
  const rp = restP.get(node);
  if (!rp) return;
  // X lateral
  const minX = rp.x - TONGUE_MAX_SIDE_M;
  const maxX = rp.x + TONGUE_MAX_SIDE_M;
  node.position.x = Math.min(maxX, Math.max(minX, node.position.x));
  // Y vertical (usa tope de caída global + subida opcional)
  const minY = rp.y - TONGUE_MAX_DOWN_M;
  const maxY = rp.y + TONGUE_MAX_UP_M;
  node.position.y = Math.min(maxY, Math.max(minY, node.position.y));
}
const _tmpW = new THREE.Vector3();
function applyTongue(t, open) {
  if (!TONGUE || TONGUE.length === 0) return;

  const s = 1 - Math.pow(1 - clamp01(open), TONGUE_EASE * 3 + 1e-6);
  if (s <= 0) { TONGUE.forEach(restore); return; }

  const parts = TONGUE;
  const L = parts.length;

  const tipRest = restP.get(parts[L - 1]) || new THREE.Vector3();

  const baseCurl = Math.min(THREE.MathUtils.degToRad(TONGUE_CFG.CURL_DEG), TONGUE_MAX_ROT_RAD) * s;
  const wagAmp = THREE.MathUtils.degToRad(TONGUE_CFG.WAG_DEG) * s;
  const wagSpd = TONGUE_CFG.WAG_SPD;

  parts.forEach((n, i) => {
    restore(n);
    const fall = (L > 1) ? (1 - i * 0.18) : 1.0;
    const curl = Math.min(baseCurl * fall, TONGUE_MAX_ROT_RAD);
    ...
    const up = (TONGUE_CFG.UP_OFFSET * s * fall);
    const fwd = (TONGUE_CFG.ALLOW_FORWARD ? Math.min(TONGUE_CFG.MAX_FORWARD, 0.006 * s * fall) : 0);
    addMov(n, 0, up, fwd);

    // Clamp local por segmento (X/Y)
    clampTongueLocalToRest(n);
  });

  // Empuje por piso de la boca + límite absoluto de caída de la punta
  const tip = parts[L - 1];
  tip.getWorldPosition(_tmpW);
  const floorY = getMouthFloorY() + TONGUE_CFG.SAFETY_CLEARANCE;
  let pushUp = 0;
  if (_tmpW.y < floorY) pushUp = Math.max(pushUp, floorY - _tmpW.y);
  const maxDropY = (restP.get(tip)?.y ?? _tmpW.y) - TONGUE_MAX_DOWN_M;
  if (_tmpW.y < maxDropY) pushUp = Math.max(pushUp, maxDropY - _tmpW.y);
